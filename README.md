...
# Flask configuration: templates in templates/, static files in static/
...
class ResumeAnalyzer:
    ...
    def analyze(self, resume_text, job_description=None):
        ...
        try:
            # Always produce job match object (for empty job descriptions results are all-zero)
            analysis['job_match'] = self.compare_with_job_description(resume_text, job_description if job_description else "")
            ...
            # Generate summary structure for easier frontend consumption
        ...
        # Report generation field for frontend and API clients
        analysis['report_text'] = self.generate_report(analysis)
        return analysis

    def generate_report(self, analysis):
        """Generate a structured text report from analysis results."""
        if not analysis:
            return "No analysis data available."

        parts = []
        parts.append(f"Overall Score: {analysis.get('overall_score', 0)}/100")
        parts.append("\nSTRENGTHS:")
        parts.extend([f" - {s}" for s in analysis.get('strengths', [])])
        parts.append("\nIMPROVEMENTS:")
        parts.extend([f" - {i}" for i in analysis.get('improvements', [])])
        parts.append("\nRECOMMENDATIONS:")
        parts.extend([f" - {r}" for r in analysis.get('recommendations', [])])
        parts.append("\nJOB MATCH:")
        job_match = analysis.get('job_match') or {}
        parts.append(f" - Match Percentage: {job_match.get('match_percentage', 0)}%")
        parts.append(f" - Matched Skills: {', '.join(job_match.get('matched_skills', [])) or 'None'}")
        parts.append(f" - Missing Skills: {', '.join(job_match.get('missing_skills', [])) or 'None'}")

        return "\n".join(parts)

    def generate_summary(self, analysis):
        """Create structured summary data for UI and API output."""
        if not analysis:
            return {
                'overall_score': 0,
        ...
        status = 'Outstanding' if score >= 80 else 'Good' if score >= 60 else 'Fair' if score >= 40 else 'Poor'

        top_strengths = analysis.get('strengths', [])[:3]
        top_improvements = analysis.get('improvements', [])[:3]
        key_actions = analysis.get('recommendations', [])[:5]

        job_match = analysis.get('job_match', {}) or {}
        matched_skills = (job_match.get('matched_tech_skills', []) or []) + (job_match.get('matched_skills', []) or [])
        missing_skills = (job_match.get('missing_tech_skills', []) or []) + (job_match.get('missing_skills', []) or [])
        ...
        return {
            ...
            'job_match_percentage': job_match.get('match_percentage', 0),
...
def generate_report():
    """Generate a downloadable report string for the current analysis"""
    data = request.get_json()
    if not data:
        return jsonify({'error': 'No analysis payload provided'}), 400
    ...
    return jsonify({'report_text': report_text}), 200
